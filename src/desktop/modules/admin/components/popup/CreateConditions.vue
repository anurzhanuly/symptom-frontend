...
import { createPopupFields } from '@desktop/utils/popUp';
import { error, success } from '@desktop/utils/toast';
...
const conditionValidation = (): boolean => {
    ...
    if (
        /\D/.test(newRecord.value.testCase) ||
        !newRecord.value.testCase.length
    ) {
...
const createCondition = () => {
    if (conditionValidation()) {
        ...
        const question: any = vals.value.filter(
...
const addValueOptions = () => {
    const options = vals.value.filter(
        (item) => item.name === newRecord.value.questionName
    );
