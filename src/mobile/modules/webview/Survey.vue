...
async function onSurveyComplete(sender: {
    ...
    try {
        const surveyResult = await webviewStore.postAnswersData(surveyAnswers);
...
onMounted(async () => {
    if (!surveyJson.value) {
        await webviewStore.getQuestionsData();
    }
});
